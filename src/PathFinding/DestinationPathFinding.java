...
public abstract class DestinationPathFinding<T> extends PathFinding<T, ArrayList<PathFindNode<T>>> {
    ...
    public ArrayList<PathFindNode<T>> calculatePath(T start, T destination) {
        ...
        if (!nodeGraph.contains(destination)) {
            return null;
        }
        traversedT.clear();
    ...
    private ArrayList<PathFindNode<T>> traverseOrigins(ArrayList<T> toTraverseInReverse) {
    ...
    private void traverseOriginsRecursive(PathFindNode<T> start) {
